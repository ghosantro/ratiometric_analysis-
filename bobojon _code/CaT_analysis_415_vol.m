    ...
    for c = 1:1:total_col_pix 
        ...
        if sum(trace1) ~=0  % If this is within the tissue 
           trace1 = reshape(trace1,1,length(trace1));  
            ...
            [butter_coef_b, butter_coef_a] = Butter_filter_design(35,50, 1.96);
            ...
            if plot_index==1
                figure; 
                hold on; 
                plot(1:1.96:1.96*length(row_filtered_sig1),trace1,'r'); 
                plot(1:1.96:1.96*length(row_filtered_sig1),row_filtered_sig1,'b'); 
                xlabel('time(ms)'); 
                ylabel('Vm'); 
...
%                     max_freq_num
                    ...
                    if length(normal_fitting_vector)>3
                        ...
                        baseline(i) = mean(pd);  % the mean value of this normal distribution will be used as the baseline value 
