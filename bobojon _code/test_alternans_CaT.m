function [alternans_index,alternans_score,peak_alternans_index,peak_alternans_score] = test_alternans_CaT(peak_value, upstroke_time_point, APD80, AP_area, known_pacing)
...
if length(APD_ratio)>=4
    ...
    if peak_alternans_index==1
        peak_alternans_score=sum(abs(peak_ratio-1))./length(APD_ratio);
    else 
