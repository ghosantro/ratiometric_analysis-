function [upstroke_time_points,rapid_depolar_end] = Get_rough_upstroke_CaT(trace,frameperiod, plot_index,row_trace,gradient_threshold,threshold)
...
index_depolar = find (gradient>gradient_threshold); % Capture the fast depolarisation phase 
subs_index = index_depolar(2:end)-index_depolar(1:end-1);
discontinious_index = find(subs_index>1)+1; %distinguish depolarisations (to get rid of littel humps set the discontinous index to 10 instead of 1)  
...
%        hist(rise_amplitude) 
%     end 
    significant_rise_index = find(rise_amplitude>threshold );   %IF THE APD MAP HAS LOTS OF HOLES CHANGE THE THRESHOLD HERE
